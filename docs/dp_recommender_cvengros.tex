...
\chapter{Introduction}
    ...
    \section{Recommender System Definition}
        ...
        \subsection{Recommender Function}
            ...
            The \emph{Recommender Function}:
            \begin{equation}
                \label{eq_recommender_function}
                R: S \times O \to [0, 1] \cup \lambda \times E
            \end{equation}
            For a given subject-object pair, $R$ gives:
            \begin{description}
                \item[Relationship prediction:] the predicted probabilitity of occurence of the given relationship type between the subject and the object. It's a number between 0 and 1 or $\lambda$, which is a special value meaining that there is already a relationship between the given pair.
                \item[Explanation:] the explanation of the prediction. For some recommenders explanations aren't available.
