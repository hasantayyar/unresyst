...
% recommendation rate nejak uzpusobit, zavest special konstantu - pro objekty ktere uz subjekt ``vlastni'' - takovy zaklad konfigurace, kdyz to ma, tak mu to nenabizim. 
    \section{Universal Recommender Architecture}
        ...
        \subsection{Representing Rules}
            ...
            \begin{enumerate}
                \item \label{ex_s_o} If a user is from a city in the south, we won't recommend him/her winter shoes.
                \item \label{ex_s_s} If two users are of aproximately the same age, they are considered similar. 
                \item \label{ex_o_o} When two pairs of shoes have in their description some common words, they are considered similar.
            \end{enumerate}
            ...
            \begin{description}
                ...
                \begin{description}
                    ...
                    \item[Object-object similarity] $D_{oo}:O \times O \to [0, 1]$ A function takes an object-object pair and determines similarity between the subjects. The higher the expectancy, the more similar the objects are, according to the rule. Example \ref{ex_o_o}: We can use a function returning the number of common words divided by the number of words in the description.
                \end{description}
                \item[Weight:] $w \in [0,1]$ A constant between $0$ and $1$ determining the strength of the rule. Weight, unlike Expectancy, is independent of the particular entities on which the rule is evalutated. In our example, the number \ref{ex_s_o} will have a high weight (e.g. $0.8$), because it is quite specific. Other rules would have lower weight, e.g. $0.1$.
                \item[Explanation:] A text explaining the meaning of the rule. Explanations can be used when presenting recommendations to users. In the text there can be gaps that that will be filled with a textual representation of entities. Our example rule \ref{ex_o_o} can have an explanation ``The shoe pairs \{object1\} and \{object2\} are similar because their descriptions contain some words in common.''
            \end{description}
        ...
        \subsection{Representing Relationships}
            ...
            Apart from defining business rules, the domain expert has to define which relationships are relevant for performing the prediction. This is done when adapting a system to Unresyst.
            ...
            \begin{description}
                \item[Subject-object relationships] indicate a subject's preference for the given object in means of the predicted relationship. In our example, if Bob has viewed a profile page of the trainers, we suppose he is quite interested in them.
                \item[Subject-subject relationships] indicate similarity among subjects. For example if Alice and Bob live in the same city they are considered similar. 
                \item[Object-object relationships] indicate similarity among objects. For example if the trainers and the rubber shoes were made by the same manufacturer they are considered kind of similar. 
            \end{description}
            ...
            \begin{description}
                ...
                \item[Explanation:] The textual explanation can be used for clarifying the meaning of the relationship. ``\{subject\} has viewed the profile of \{object\}.
