    ...
    \section{Definitions}
        ...
        \label{basic_terms}
            ...
            \begin{description}
                ...
                \item[Preference in means of the predicted relationship]: For predicting a relationship we need a rate - a number determining the estimated probability that the \emph{predicted relationship} will ocurr between the subject and object. In our user-item \emph{like} example, the preference would mean how much the user is likely to like the item. 
             \end{description}
...
% recommendation rate nejak uzpusobit, zavest special konstantu - pro objekty ktere uz subjekt ``vlastni'' - takovy zaklad konfigurace, kdyz to ma, tak mu to nenabizim. 
    \section{Universal Recommender Architecture}
        ...
        \subsection{Representing Rules}
            ...
            The RACOFI framework \cite{racofi} uses two types of actions: \emph{Modify} that modifies the predicted subject's object rating by a given constant and \emph{Not Offered} that excludes given object from recommendation to subject.
            
            Our rules aren't modifying predictions, but they serve as an input for predicting. Therefore we will use absolute values determining subject-object preference in means of the recommended relationship, not relative values as in RACOFI \emph{Modify} rules. The \emph{Not Offered} rule type can be represented by a rule giving the minimal possible value, so we won't have a special rule type for it. 
            ...
            A better option would be to implement our own \emph{Object-oriented rule representation}, where we represent rules as instances of a rule class. In the addaptation phase, the domain expert instantiates the pre-defined class to create a business rule.
             
            For our object-oriented rule representation we need to define a class which instances hold conditions, actions and some additional data. As we have showed, the only possible action is determining the preference or inter-entity similarity. Therefore actions will be represented as functions giving a preference or similarity respectively. More precisely, the rule objects contain:
            \begin{description}
                ...
                \end{description}
                \item[Confidence:] A function determining the strength of the preference/similarity.
                \begin{description}
                    \item[Subject-object preference confidence] $D_{so }:S \times O \to [0,1]$. A function takes a subject-object pair and estimates a probability, that the \emph{predicted relationship} appears between the subject and the object. Values close to $1$ mean that, according to the rule, the object should be recommended to the subject. Low values close to $0$ mean that the object shouldn't be recommended to the object. Values in the middle (near $0.5$) mean that the rule is unsure about recommending the given object to the given subject - that makes a similar effect as if the condition $C$ wasn't satisfied for the subject-object pair. Example: rating nebo has clicked kolik sekund.
                    \item[Subject-subject similarity] $D_{ss}:S \times S \to [0,1]$. A function takes a subject-subject pair and determines similarity between the subjects. The higher the confidence, the more similar the subjects are, according to the rule. Example: friends
                    \item[Object-object similarity] $D_{oo}:O \times O \to [0, 1]$ A function takes an object-object pair and determines similarity between the subjects. The higher the confidence, the more similar the subjects are, according to the rule. Example: the same color
                \end{description}
                \item[Weight:] $w \in [0,1]$ A constant between $0$ and $1$ determining the strength of the rule. Weight, unlike Confidence, is independent of the particular entities on which the rule is evalutated. Example: mys pres bude mit malou vahu, klik vetsi, explicitni hodnoceni nejvetsi.
        ...
        \subsection{Representing Relationships}
            napsat neco o ekvivalenci relationships a rules - spis ze vsechny relationships jdou napsat jako rules
            
            Apart from business rules, when adapting a system to Unresyst, the domain expert has to define which relationships are important for making recommendations.
