...
\chapter{Introduction}
    ...
    \section{Definitions}
        ...
        \subsection{Relationship Prediction}
            ...
            \begin{description}
                \item[Expectancy:] the predicted probabilitity of occurence of the given relationship type between the subject and the object. It's a number between $0$ and $1$. The value $1$ is reserved for the subject-object pairs, that are already connected by the relationship of the given type. Thus, the objects with relationship expectancy $1$ aren't presented to the subject. The recommended objects are usually presented ordered by the expectancy. The exact expectancy value doesn't have to be presented to the subject.
            ...
            In a universal recommender we can't assume there will be a rating relationship between subjects and objects. Moreover, we are supposed to be able to predict any given relationship between subjects and objects, not just the preference expressed by the rating. In a Universal recommender, we only predict a $0/1$ indicator: the predicted relationship is between the given the subject and object, or it isn't. Hence we can shrink the prediction and confidence concepts into one - relationship expectance. 
            
            Let's show the reduction on an example. A recommender predicts, that \emph{Bob} will like the \emph{Five Days In Paris} book with $20\%$ probability. The like expectancy is $0.2$, which can be rewritten as giving prediction $1$ (Bob will like it) with the prediction confidence $0.2$. For us, this is the same as predicting that Bob won't like it with $80\%$ probability - giving a prediction $0$ with prediction confidence $0.8$. This is because, in our relationship representation, Bob either likes the book, or he doesn't. The events ``Bob likes the book'' and ``Bob doesn't like the book'' are complementary.
            
            In other words, low relationship expectance can mean low prediction value with high confidence as well as high prediction value with low confidence. But as the predicted indicator is of a $0/1$ type, we don't have to differentiate these cases. 
            ...
            The dependency between relationship expectancy and prediction confidence is illustrated on the figure \ref{pic_expectancy_confidence}. Let prediction confidence be a function to $[0,1]$ determining how sure the recommender is about the given relationship prediction. Expectancy values just above zero indicate that the recommender is quite sure that there won't be the given relationship between the given subject-object pair. Expectancy values near $0.5$ mean low confidence for the prediction. Especially $0.5$ means that according to the recommender, the probability of the relationship occurence is the same as for non-occurence. Expectancy values close to $1$ mean that recommender is quite sure, that there be the given subject-object pair will be connected by the given relationship. 

            \fig[9]{pics/expectancy_confidence.png}{The dependency between relationship expectancy and prediction confidecne.}{pic_expectancy_confidence}
...
% recommendation rate nejak uzpusobit, zavest special konstantu - pro objekty ktere uz subjekt ``vlastni'' - takovy zaklad konfigurace, kdyz to ma, tak mu to nenabizim. 
    \section{Universal Recommender Architecture}
        ...
        \subsection{Representing Rules}
            ...
            Our rules aren't modifying predictions, but they serve as an input for predicting. Therefore we will use absolute  relationship expectancy, not relative values as in RACOFI \emph{Modify} rules. The \emph{Not Offered} rule type can be represented by a rule giving the minimal possible expectancy, so we won't have a special rule type for it. 
            ...
            A better option is to implement our own \emph{Object-oriented rule representation}, where we represent rules as instances of a rule class. In the addaptation phase, the domain expert instantiates the pre-defined class to create a business rule.
             
            For our object-oriented rule representation we need to define a class which instances hold conditions, actions and some additional data. As we have showed, the only possible actions are determining the expectancy of the subject-object relationship and determining inter-entity similarity. Therefore actions will be represented as functions giving an expectancy or similarity respectively. More precisely, the rule objects contain:
            \begin{description}
                \item[Condition:] A boolean function determining whether the rule can be applied to a given entity pair. The function has a form depending on the rule type:
                \begin{description}
                    \item[Subject-object condition] $C_{so}:S \times O \to \{False, True\}$. A function takes a subject-object pair and determines whether the rule can be applied to the pair. Example: rating nebo has clicked, liked s oklikou pres entitu jineho typu - treba tag
                    \item[Subject-subject condition] $C_{ss}:S \times S \to \{False, True\}$. A function takes a subject-subject pair and determines whether the rule can be applied to the pair. Example: friends
                    \item[Object-object condition] $C_{oo}:O \times O \to \{False, True\}$. A function takes an object-object pair and determines whether the rule can be applied to the pair. Example: the same color
                \end{description}
                \item[Expectancy:] A function determining the strength of the subject-object preference in means of the predicted relationship or subject-subject/object-object similarity.
                \begin{description}
                    \item[Subject-object relationship expectancy] $D_{so }:S \times O \to [0,1]$. A function takes a subject-object pair and estimates a probability, that the \emph{predicted relationship} appears between the subject and the object. Values close to $1$ mean that, according to the rule, the object should be recommended to the subject. Low values close to $0$ mean that the object shouldn't be recommended to the object. Values in the middle (near $0.5$) mean that the rule is unsure about recommending the given object to the given subject - that makes a similar effect as if the condition $C$ wasn't satisfied for the subject-object pair. Example: rating nebo has clicked kolik sekund.
                    \item[Subject-subject similarity] $D_{ss}:S \times S \to [0,1]$. A function takes a subject-subject pair and determines similarity between the subjects. The higher the expectancy, the more similar the subjects are, according to the rule. Example: friends
                    \item[Object-object similarity] $D_{oo}:O \times O \to [0, 1]$ A function takes an object-object pair and determines similarity between the subjects. The higher the expectancy, the more similar the objects are, according to the rule. Example: the same color
                \end{description}
                \item[Weight:] $w \in [0,1]$ A constant between $0$ and $1$ determining the strength of the rule. Weight, unlike Expectancy, is independent of the particular entities on which the rule is evalutated. Example: mys pres bude mit malou vahu, klik vetsi, explicitni hodnoceni nejvetsi.
                \item[Explanation:] A text explaining the meaning of the rule. Explanations can be used when presenting recommendations to users. In the text there can be gaps that that will be filled with a textual representation of entities. Example treba ty friends. 
            \end{description}
        ...
        \subsection{Representing Relationships}
            ...
            Apart from defining business rules, the domain expert has to define which relatioship should be predicted and which relationships are relevant for performing this prediction. These actions are performed when adapting a system to Unresyst.
            
            We have some specific requirements on the relationship representation. In the system there can be various entities, not only subjects and objects. The relationships can also span such entities. Example: XXX Secondly, the relationships can have various relevance for predicting the given relationship. This should be implemented by assigning weights to relationships. For example XXXX
            
            We would like to represent relationships of the following types. The relationships don't have to be direct - on the path from subject/object to subject/object there can be an entity of any type. Example XXXX
            \begin{description}
                \item[Subject-object relationships] indicate a subject's preference for the given object in means of the predicted relationship. For example: XXXX
                \item[Subject-subject relationships] indicate similarity among subjects. For example: XXX velikost nohy
                \item[Object-object relationships] indicate similarity among objects. For example: XXX barva tricka
            \end{description}
            ...
            \begin{description}
                \item[Condition:] The boolean function means whether the given subjects/objects are in the relationship. The forms of the function remain the same as described in the section \ref{representing_rules}. Example XXXX.
                \item[Expectancy:] The function won't be needed for representing relationships. If the Condition function returns $True$ for a subject/object pair the pair is surely in a relationship. Hence, such a function would always return $1$ for relationhips.
                \item[Weight:] We need a way how to define the relevance of the relationship for predicting the relationship. Weight can be used on this purpose. Example XXXX. 
                \item[Explanation:] The textual explanation can be used for clarifying the meaning of the relationship. Example XXXX. 
                 
             \end{description}
            
            Such a representation can use semantics of a system independently of its data model. There doesn't have to be a direct link between subjects/objects in the data model to define a relationship (Example XXXX). Moreover, the relationships can be taken from various sources, e.g. multiple databases, data stores, or log files. The only thing that has to be provided to Unresyst is an implementation of a function returning whether the given subjects/objects are in the relationship.
            
            This representation is a reduction of the full semantic representation used in \cite{white_paper}. The authors propose using all system entities and relationships for recommending. This approach has some significant drawbacks. Firstly, it's not clear how to assign weights for relationships between entities that are not subjects or objects. Such relationship can have a varied meaning in the context of different relationships. Example: XXXX. Secondly, most of the main-stream recommender algorithms use only subjects, objects and relationships between them. Therefore the choice of a recommender algorithm is much more limited.
            
            The relationships between entities that are not subjects or objects can be used in our representation as well, but only those that are on a path from a subject to an object, or subject to subject or object to object. Example: XXXX To our relationship definition we can include a relationship of any type:
            \begin{description}
                \item[One-to-one relationships] can be included by directly traversing the relationship. Example in pseudocode. XXXX
                \item[One-to-many relationships] can be included by looping through all related entities of an entity (Example in pseudocode XXXX)  or testing whether a given entity is in the related entities of another entity Example in pseudocode XXXX).
                \item[Many-to-many relationships] can be included by as one-to-many relationships Example in pseudocode XXXX.
             \end{description}
             ...
             \subsection{Representing Relationship to Predict}
                ...
                This is done in a way similar to defining relationships that are relevant for recommendations. The only attribute that can be ommited is the \emph{weight}. The condition and the explanation remains as described in the section \ref{representing_relationships}.
                
                Example in pseudocode XXXX.
