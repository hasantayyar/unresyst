...
\centerline{\mbox{\includegraphics[width=60mm]{pics/logo.jpg}}}
...
%%% Strana s èestným prohlá¹ením k diplomové práci
...
\vedouci, \katedracesky
% dle Organizaèní struktury MFF UK, pøípadnì plný název pracovi¹tì mimo MFF UK

Abstrakt:
Doporuèovací systémy jsou programy, které se u¾ivateli nabízejí  \mbox{objekty} (napø. knihy nebo hudbu), které by pro nìj mohly být zajímavé. Tyto systémy získávají vzrùstající popularitu a jsou intenzivnì studovány výzkumnými skupinami po celém svìtì. Ve webových systémech, jako jsou internetové obchody nebo komunitní servery, bývají obvykle k dispozici rùzné datové zdroje, které mohou být vyu¾ity k doporuèování, napø. atributy u¾ivatelù a objektù, hodnocení objektù u¾ivateli nebo nepøímá zpìtná vazba získaná ze zaznamenaného chování u¾ivatele. V této práci pøedstavujeme koncept Univerzálního doporuèovacího systému (Unresyst), který doká¾e vyu¾ít tìchto datových zdrojù a zároveò je doménovì nezávislý. V práci navrhujeme zpùsoby vyu¾ití systému Unresyst, ze souèasných metod pou¾ívaných k doporuèování vybíráme jako nejvíce vhodnou knowledge-based metodu kombinovanou s kolaborativním filtrováním. Dále \mbox{analyzujeme} datové zdroje v rùzných systémech a zobecòujeme je tak, aby byly doménovì nezávislé. Navrhujeme architekturu systému Unresyst, popisujeme rozhraní systému a zpùsoby zpracování datových zdrojù. Dále pøizpùsobujeme Unresyst na tøi data sety z reálných systémù, vyhodnocujeme pøesnost doporuèení a srovnáváme ji se souèasnými algoritmy pro kolaborativní filtrování. Srovnání ukazuje, ¾e kombinování rùzných datových zdrojù mù¾e zlep¹it pøesnost doporuèovaní kolaborativního filtrování a mù¾e být pou¾ito v systémech, pro které standardní kolaborativní není vhodné.


Klíèová slova:
Doporuèovací systémy, Internetové obchodování, Doménová nezávislost, Knowledge-based doporuèování, Kolaborativní filtrování
...
Keywords:
Recommender Systems, E-commerce, Domain-Independence, Knowledge-Based Recommender, Collaborative Filtering
...
\chapter{Introduction}
    ...
    \section{Thesis Structure}
        ...
        In the chapter \emph{Verifying the Universal Recommender on Real-World Data} we describe how we adapted the Unresyst recommender to data sets containing real data. 
        
        In the chapter \emph{Evaluating Recommender Results} we propose off-line methods and metrics that can be used for evaluating a universal recommender. We describe the results obtained for Unresyst prototype on the studied data sets and propose methods on how the results can be improved.
    ...
    \section{Thesis Concepts}
        ...
        \subsection{Motivation}
            ...
            Using multiple sources of data for recommending is also a current trend in the recommender research field, as illustrated by the current recommender system competition, organized by members of the Netflix prize winning team\footnote{See the \emph{KDD cup recommender competition} at \url{http://kddcup.yahoo.com/index.php} }.
            ...
            The obtained \emph{compiled predictions} can be used directly for recommending or they can serve as an input for an ordinary recommender algorithm. More about recommending in the Last.fm data set can be found in chapter \ref{verifying_last_fm}. 
        ...
        \subsection{Thesis Classification}
            \label{classification}
                ...
                The recommender system research field can be divided into the following categories, as described in the paper \cite{probabilistic_analysis}:
                ...
                \begin{description}
                    \item[Scalable implementation] The provided recommender application is meant as a prototype and isn't intended to be scalable for large data sets. 
...
\chapter{Analysis}
    ...
    \section{Definitions}
        ...
        \label{def_recommender_system}
            ...
            Similarly to the recommender problem definition in the recommender system survey \cite{survey}, we formally define the recommendation as choosing the object $o_s$ for each subject $s$, so that the object maximizes the subject's utility function $u_R$:
            \begin{equation}
        ...
        \subsection{Relationship Prediction}
            ...
            For instance, our recommender predicts that $Bob$'s listening to $Beck$ utility is $0.8$, because $Beck$ is similar to $Sonic \ Youth$, a band that $Bob$ has listened yesterday. In our notation: $\hat{u_R}(Bob, Beck)=(0.8,$ ``Bob  has listened to Sonic Youth band, which is similar to Beck''$)$. 
    ...
    \section{Operations in Recommender Systems}
        ...
        \begin{description}
            \item[Recommender build] After a configuration of the recommender is done, the recommender has to prepare itself for recommending objects. This operation may be pretty time consuming, as this doesn't have to be performed very often. 
            \item[Recommendation] Recommendation corresponds to evaluating the function $\rho^{\hat{u}_R}$ for a given subject. Recommendation is the most common operation for a recommender system. Choosing objects with the highest expectancy for the given subject among all objects can be a very expensive operation. That's why most recommender systems introduce some kind of index, enabling the recommender to recommend objects real-time. This operation should be performed very fast, ideally in a constant time independent of the number of items in the system.
...
\chapter{Recommender Algorithms}
    ...
    \section{Collaborative Filtering}
        ...
        More formally, classic user-based collaborative filtering methods use a function $\hat{u}^{CCF}_R$ for estimating the utility as  depicted in the formula \ref{eq_cf}. In the formula, $v_R: S \to \mathbb{R}^{|O|}$ is a function giving a subject vector to a given subject $s$, according the objects that are in $R$ with $s$. $cos$ is cosine of two vectors, as defined e.g. in \cite{survey}.
        ...
        In general, collaborative filtering methods are currently the most used and the most successful methods for recommending. When properly used, they provide high accuracy and scalability for large amount of data. However they have some drawbacks too \cite{survey}.
        \begin{description}
            \item[Cold start problem] As for the content-based method, the system doesn't know any liked objects for a new subject. For a user being a subject, this problem can be solved by asking the user to enter some data. This approach is used in the Netflix system. A new user is given a questionnaire for rating some chosen movies \cite{netflix_solution}. Another solution to the problem is using a hybrid recommender (\ref{hybrid_recommenders}) that would use an easier recommender (like giving the most popular objects) when predictions from collaborative filtering aren't available.
    ...
    \section{Knowledge-based Recommenders}
        ...
        According to \cite{knowledge_spain}, there are three types of knowledge, a recommender system can deal with:
        \begin{description}
            \item[Catalogue knowledge] provides information about objects and their features.
    ...
    \section{Hybrid Recommenders}
        ...
        More formally, a linear weighted hybrid recommender, as described in \cite{burke_hybrid}, uses $\hat{u}^{H}_R$ for estimating the utility, as depicted in the formula \ref{eq_df}. $U$ is a set of functions belonging to the contained recommenders, $\hat{u'}_{iR} \in U$ for $i=1, \dots |U|$. $w_i \in [0,1]$ is a relative weight of the utility function $\hat{u'}_{iR}$.
...
\chapter{Relationships in Studied Systems}
    ...
    In the chapter we analyze selected systems where the Unresyst recommender could be used. As we don't have a direct access to the system databases, we use data sets\footnote{Data set is a set of text files containing some part of data included in the system database}. The first two data sets - Last.fm and Flixster are publicly available. The third one (Czech travel agency e-shop data set) was made accessible to the thesis author by the courtesy of the system administrator, who didn't wish to disclose his name.
    \section{The Last.fm Dataset}
        \label{rel_last_fm}
        Last.fm is a music recommendation service \cite{last_fm}. The system users post the information about what tracks they're listening to through plugins installed in their music players. Last.fm provides a freely available API\footnote{See \url{http://www.last.fm/api}.} for developers who can build applications using Last.fm data. 
        
        \fig[14]{pics/lastfm.png}{The data model of the Last.fm data set}{pic_last_fm}
        
        The API was used for crawling the data sets we used for our study. The first used data set \cite{last_fm_dataset1} contains users and their listening history. Each user has listened and posted (\emph{scrobbled}) some tracks, each track was recorded by an artist. For some of the users there's some basic personal information available (age, gender and home country), each scrobble has a timestamp. 
        
        The data set was integrated with the Last.fm social tag data set \cite{last_fm_dataset2}. Last.fm users can tag artists, describing the genre the artist is playing (like \emph{Pop}, \emph{Rock} or \emph{Techno}), the feeling of the music (like \emph{slow}, \emph{loud}) or any other property. For some of the artists we were able to assign tags, the users gave the artist.
        
        The mentioned data sets were selected because together they contain basic attributes of both subjects and objects, which can be used for similarity measures. It contains implicit feedback (the scrobbles) that discloses some positive preferences of the users. The data model of available data is displayed in the figure \ref{pic_last_fm}

    \section{The Flixster Data Set}
        Flixster is a community server for sharing movie reviews and ratings\footnote{See \url{http://www.flixster.com/}.}. Each contained movie has a profile page where registered users can comment and rate the movie. Users can become friends with each other. The Flixster data set \cite{flixster_dataset} doesn't contain any properties for movies nor for users. The only data available is the user-movie rating and links between users who are friends. The data model is displayed in the figure \ref{pic_flixster}.
        
        \fig[8]{pics/flixster.png}{The data model of the Flixster data set}{pic_flixster}
    ...
    \section{The Travel Agency Data Set}
        ...
        \fig[15]{pics/travel2.png}{The data model of the Travel Agency data set}{pic_travel}
    ...
    \section{Summary}
        ...
        The studied systems were selected so that they cover a variety of different domains, where we can recommend. The data sets fulfill the recommender applicability conditions described in the section \ref{recommender_system_applicability}. The systems were selected so that we can prove the usefulness of the Unresyst recommender concept for processing both implicit and explicit feedback. Some of the data sets have attributes for their subjects and objects, that can be used for similarity measures.
        
        The rules, relationships and biases used in Unresyst for the described data sets are presented in the Chapter \ref{adapting_to_existing_systems}.
...
\chapter{Universal Recommender Design and Implementation}
    ...
    \section{Overview of the Universal Recommender Architecture}
        \label{architecture}
        ...
        The figure \ref{pic_unresyst_adaptation} shows how Unresyst can be adapted to a domain on the example of the music domain. In the example we create two recommenders for a domain. A domain expert firstly creates the desired recommenders. In the example it's a \emph{Novel Artist Recommender} and a \emph{Artist Radio Recommender}. Both recommenders suggest artists to listeners. The Novel Artist recommender tries to broaden listener's music horizons by suggesting artists the listener haven't heard yet. Whereas the Artist Radio Recommender suggests artists to a listener no matter if he/she has already heard it. Such a recommender can be used for a personalized internet radio.
...
class ArtistRecommender(Recommender):
    ...
    """The objects that will be recommended."""
            ...
            \subsubsection{Representing Rules}
                ...
                \begin{enumerate}
                    \item \label{ex_s_o} \emph{Subject-object rules} defining subject-object interactions (preferences) that can be used for recommending objects to subjects. We denote $RL_{pref}$ a set of subject-object rules for a recommender. An example of a negative subject-object rule is the mentioned ``Music having the given tags shouldn't be recommended to users of the given gender''.
                    \item \label{ex_s_s} \emph{Subject-subject rules} defining similarity between subjects. We denote $RL_{ssim}$ a set of subject similarity rules for a recommender. An example of a positive similarity rule is ``Users of similar age are similar''.
                    \item \label{ex_o_o} \emph{Object-object rules} defining similarity between objects. We denote $RL_{osim}$ a set of object similarity rules for a recommender. An example of a positive similarity rule is the mentioned ``Artists sharing some of their tags are similar''.
                \end{enumerate}
                
                There are several ways how to represent rules. The first studied one is \emph{RuleML (Rule Markup Language)}\footnote{See \url{http://ruleml.org/}.}, a XML-based markup language for representing rules. The language is very powerful, having the same expressiveness as declarative programming languages\footnote{There are several converters between the \emph{Prolog} programming language and RuleML, e.g. \emph{$W^4$ RuleML Compiler}, see \url{http://centria.di.fct.unl.pt/~cd/projectos/w4/ruleml/index.htm}.}. RuleML was used for entering rules into the RACOFI recommender system \cite{racofi}.
...
dont_recommend(U, A) :- 
    ...
    male_specific(T).
                ...
                An option better fitting our needs is to implement our own \emph{Object-oriented rule representation}, where we represent rules as instances of a rule class. In the adaptation phase, the domain expert instantiates the predefined classes to create business rules.
                ...
                \begin{description}
                    ...
                    \begin{description}
                        \item[Subject-object condition] $C_{pref}:S \times O \to \{False, True\}$ for rules from $RL_{pref}$. A function takes a subject-object pair and determines whether the rule can be applied to the pair. In our example \ref{ex_s_o} the condition function is: If the user $s$ is a female and artist $o$ is tagged by one of the given tags, the condition evaluates to \emph{True}. Otherwise it's \emph{False}
                        \item[Subject-subject condition] $C_{ssim}:S \times S \to \{False, True\}$ for rules from $RL_{ssim}$. A function takes a subject-subject pair and determines whether the rule can be applied to the pair. In our example \ref{ex_s_s} the condition function is: If we know the age of both of the users, the condition is \emph{True}, otherwise it's \emph{False}.
                        \item[Object-object condition] $C_{osim}:O \times O \to \{False, True\}$ for rules from $RL_{osim}$. A function takes an object-object pair and determines whether the rule can be applied to the pair. In our example \ref{ex_o_o} the condition function is: If the artists share some tags, the condition is \emph{True}, otherwise it's \emph{False}. 
                    \end{description}
                ...
                The following pseudo-code, shows evaluating a rule $r \in RL_{pref}$ for a subject $s$ and object $o$. If the condition is evaluated to $True$, the confidence is counted and positive or negative preference is stored according to the positiveness of the rule.
...
End If
                ...
                Let's show a representation of our example rule $r_1 \in RL_{pref}$ \emph{Don't recommend male-specific artists to females}. 
                
                The formula \ref{eq_ex1_cond} depicts the condition of the example rule. $F \subseteq S$ is a set of female subjects, $MST \subseteq T$ is a set of male specific tags, a subset of the set of tags $T$ and $AT \subseteq O \times T$ is a relation between artists and tags, meaning the artist was tagged by the given tag.
                
                \begin{equation}
                    \label{eq_ex1_cond}
                    C^{r_1}_{pref}(s, o) = \left\{\begin{array}{ll}
                            True & \textrm{if $s \in F$ and $\exists t \in MST, (o, t) \in AT$} \\
                            & \\
                            False & \textrm{otherwise} \\
                        \end{array} \right.
                \end{equation}
                
                The formula \ref{eq_ex1_conf} depicts the confidence function of the rule. The confidence is the proportion of the male-specific tags to all tags given to the artist.
                \begin{equation}
                    \label{eq_ex1_conf}
                    D^{r_1}_{pref}(s, o) = \frac{|\{(o, t) |\  t \in MST \}|}{|\{(o, t) |\  t \in T\}|}
                \end{equation}

                
                The following lines of code show the example rule $r_1 \in RL_{pref}$ written in our Python prototype recommender implementation. Weight is set to a chosen constant, it can be replaced by a more appropriate one as described in the section \ref{improving_the_results}. The character sequences \verb!%(object)s! and \verb!%(subject)s! are placeholders for the name of the object and subject respectively. When presenting recommendations to a user, these placeholders are filled with the actual object resp. subject name. 
...
class ArtistRecommender(Recommender):
    ...
    rules = (
        ...
        \end{verbatim}
                ...
                Such rules can represent each of knowledge types described in \cite{knowledge_spain}, described in the section \ref{knowledge_based}. Catalogue knowledge can be represented by rules accessing object properties in their Condition and Confidence functions. Functional and user knowledge can be representing by rules using the relationships between subjects and objects. 
            ...
            \subsubsection{Representing Relationships}
                ...
                Let's show the relationship representation on our example relationship $r_2 \in RL_{ssim}$ \emph{If two users are from the same country, they are similar}.
                
                The formula \ref{eq_ex2_cond} depicts the condition function of the rule $r_2$, where $country: S \to C \cup \{\varnothing\}$ is a function that gives a country from the set $C$ to a given user, or $\varnothing$ if the user hasn't filled his/her country.
                
                \begin{equation}
                    \label{eq_ex2_cond}
                    C^{r_2}_{ssim}(s_1, s_2) = \left\{\begin{array}{ll}
                            True & \textrm{if $country(s_1) \neq \varnothing $ and $country(s_2) \neq \varnothing $} \\
                            & \textrm{and $country(s_1) = country(s_2)$}\\
                            & \\
                            False & \textrm{otherwise} \\
                        \end{array} \right.
                \end{equation}
        ...
        SubjectSimilarityRelationship(
            ...
            " are from the same country.",
                ...
                \subsubsection{Representing Predicted Relationship}
                    ...
                    The formula \ref{eq_ex_pred} depicts the condition function of the predicted relationship $pr$, where $scr: S \times O \to \textbf{N}$ is a function that for a user $s$ an artist $o$ gives the number of scrobbles (how many time the user played artist's songs).
                
                    \begin{equation}
                        \label{eq_ex_pred}
                        D^{pr}_{pred}(s, o) = \left\{\begin{array}{ll}
                                True & \textrm{if $scr(s, o) > 0$} \\
                                & \\
                                False & \textrm{otherwise} \\
                            \end{array} \right.
                    \end{equation}
    ...
    """The relationship that will be predicted"""
                ...
                \subsubsection{Representing Biases}
                    ...
                    \begin{description}
                        ...
                        \begin{description}
                            \item[Object bias condition] $C_{bobj}:O \to \{False, True\}$. A function takes an object and determines whether the object is influenced by the bias. In our example  the condition function is: If the artist $o$ is played more than $N$ times in a given recent period, the condition evaluates to \emph{True}. Otherwise it's \emph{False}
                            \item[Subject bias condition] $C_{bsubj}:S \to \{False, True\}$. A function takes a subject and determines whether the subject is influenced by the bias.
                        \end{description}
                        ...
                        \begin{description}
                            \item[Object bias confidence] $D_{bobj}:S \to [0,1]$. A function takes an object  and gives a confidence, that the object will be preferred by any given subject (or not preferred when negative). High values mean strong confidence. Low values mean that the rule is unsure about the impact (positive or negative) on the preference. In our example we take a confidence function returning the artist play count divided by the maximum play count for an artist in the given period.\footnote{For the sake of simplicity we take an absolute play count and a fixed time period. For better results it would be better to take the number of plays relative to an average artist play count in a given period. Our example function would probably give high biases to a constant set of popular artists.}

                            \item[Subject bias confidence] $D_{bsubj}:S \to [0,1]$. A function takes a subject and gives a confidence, that the subject will prefer any given object (or not prefer when negative). 
                        \end{description}
                         \item[Weight, Description] The properties has the same meaning as in rule definition (see \ref{representing_rules}).
                    \end{description}
                    
                    Let's show the bias representation on an example bias $b \in B_{obj}$ \emph{Artists that were much played recently are likely to be played}.
                    
                    The formula \ref{eq_ex_b_cond} depicts the condition function of bias $b$, where $scr_P: S \times O \to \textbf{N}$ is a function that for a user $s$ an artist $o$ gives the number of scrobbles in the given time period $P$ (how many time the user played artist's songs in the period), $mincount \in \textbf{N}$ is a constant determining the lower bound for applying the bias.
                    
                    \begin{equation}
                        \label{eq_ex_b_cond}
                        C^{b}_{bobj}(o) = \left\{\begin{array}{ll}
                                True & \textrm{if $\sum_{s' \in S} scr_P(s', o) > mincount$} \\
                                & \\
                                False & \textrm{otherwise} \\
                            \end{array} \right.
                    \end{equation}
                    
                    The formula \ref{eq_ex_b_conf} depicts the confidence function of bias $b$, where 
                    $maxcount \in \textbf{N}$ is a constant.
                    \begin{equation}
                        \label{eq_ex_b_conf}
                        D^{b}_{bobj}(o) = \frac{\sum_{s' \in S} scr_P(s', o)}{maxcount}
                    \end{equation}
            ...
            confidence=lambda artist: \
                ...
                \subsection{Runtime Interface}            
                    ...
                    \begin{description}
                        ...
                        occurring between the subject and the object, which is a rate of subject-object preference. The attribute contains a float number between from the $[0, 1]$ interval.
                        \item[$explanation$] A short textual explanation of the provided prediction. The attribute contains a short text that can be presented to the user. For some algorithms this attribute may be unavailable.
                    \end{description}
                    ...
                    The usage of the runtime interface of our Python prototype implementation can be seen on the following lines. Firstly we import all necessary classes and build the recommender. The \verb!User! model points to a table in the \verb!lastfm! application containing system users, the \verb!Artist! model table contains artists. These are domain-specific models belonging to the application. Using the recommender method \verb!get_recommendations! we obtain one artist that is according to the recommender most likely to be liked by the user $44$. The recommender proposes artist \emph{Cat Stevens}, with expectancy $0.75$. The recommender explains, that the user $44$ is similar to user $2$, because they're both females, and the user $2$ has already listened to Cat Stevens. 
                     \begin{verbatim}
...
to the Boards Of Canada's tracks. >
    ...
    \section{Applying Rules}
        ...
        \subsection{Matching Rule Conditions to Entities and Entity Pairs}
            ...
            Matching the entities and entity pairs to the given conditions is done by substituting all possible entities and entity pairs one by one to the given conditions. As the complexity of this action is $O(mn)$ ($m$ is the number of subjects, $n$ the number of objects), in our prototype implementation we enable defining a \emph{generator} function for each rule, relationship or bias (see \ref{unresyst_implementation_details} for details).
        ...
        \subsection{Combining}
            ...
            \begin{enumerate}
                \item If one element is positive and the other negative, the result should be neutral. \label{pos_neg}
            ...
            \subsubsection{Certainty Factor Calculus}
                ...
                Finally, our resulting function that combines two expectancies to a single expectancy $CMB: [0,1] \times [0, 1] \rightarrow [0,1]$ is in \ref{eq_cf_final}.
                \begin{equation}
                    \label{eq_cf_final}
                    CMB(e_1, e2) = t^{-1}(CFC( t(e_1), t(e_2)))
                \end{equation}

                As noted in \cite{certainty_calculus}, the function is commutative and associative in its second argument, so the order of the combination has no effect on the result. The resulting function apparently fulfills the requirements for our combination function.
        ...
        \section{Unresyst Application Layers}
            ...
            \subsection{Recommender Layer}
                ...
                The top most level (\emph{Recommender}) provides the runtime interface (see \ref{runtime_interface}) to the whole recommender, the adaptation data is stored in the specific recommender subclass (see \ref{adaptation_interface}). 
            
            
            \subsection{Abstractor Layer}
                ...
                Our prototype implementation follows the proposed architecture. The classes of the recommender follow the proposed layers. The final configuration of classes that will be used as the proposed layers is done right in the \emph{Recommender} base class, so that it can be overridden in its specific subclasses.
            ...
            \subsection{Algorithm Layer}
                ...
                \begin{description}
                    ...
                    \item[Compiling algorithm level] The level compiles the subject-object rules, aggregated similarities and aggregated biases to relationship predictions during the build and update, see \ref{compiling} for details. For combining multiple sources of prediction it uses the Combinator module (\ref{combinator}). The input of the level are subject-object rules, aggregated similarities and aggregated biases. The output of the level are compiled subject-object preference predictions.
                    \item[Exploiting algorithm level] The level takes the predictions obtained from the higher level and later exploits them for other pairs. For this layer, we can use a classical collaborative filtering algorithm as described in \ref{collaborative_filtering}. In the prototype implementation we use only a simple algorithm returning predictions it has obtained from the compiling level. The input of the level are the predictions, the output are the obtained predictions, optionally exploited to some previously unpredicted subject-object pairs.
                \end{description}
        ...
        \section{Unresyst Prototype Implementation}
            ...
            \subsection{Unresyst Prototype Data Model}
                ...
                The data model for our simple algorithm is in the figure \ref{pic_algorithm_models}. As our implementation only returns the results of the levels above, it only contains a model for preference prediction. These models aren't generated for all possible pairs as this wouldn't be feasible for larger domains. However, as we need to generate recommendations for a given subject fast, we do some pre-computation. During the build we select the given number of promising objects for each subject. The promising objects are chosen from all rules, relationships and biases that are available. For these objects, all known preference sources are inspected (see \ref{compiling}) and the predictions for the pairs are saved.
            ...
            \subsection{Implementation Details}
                ...
                \begin{enumerate}
                    ...
                    \item If it's a similarity relationship we save the entity with the lower private key to the first position and the entity with the higher private key as the second.
                \end{enumerate}
...
\chapter{Verifying the Universal Recommender on Real-World Data}
    ...
    In the chapter we describe the process of adapting the Unresyst prototype to the selected data sets (see Chapter \ref{relationships_in_studied}). 
    ...
    \section{Last.fm}
        ...
        We decided to predict the relationship \emph{User listens to Artist}, rather than predicting the listened tracks. That's because the range of played tracks from each artist is pretty wide and different users tend to listen different tracks.
        ...
        \begin{itemize}
            \item Similarity of users according to their age. We take only pairs of users whose age difference is lower than a given constant. The confidence of similarity raises with lowering age difference. We suppose, the users of similar age are likely to listen to the same artists.
            ...
            \item Positive bias for popular artists. Artists that have been listened by many users (more than a given constant) get a positive bias. We suppose that artists, that were listened by a high number of users, are likely to be liked by any user. 
    ...
    \section{Flixster data set}
        ...
        \begin{itemize}
            ...
            \item Subject bias for high- and low-rating users. As mentioned in the section \ref{representing_biases}, the subject biases are relevant only in cases when we care about the exact expectancy number. This is the case of the Flixster data set as we try to predict the exact user - movie rating. Some users tend to rate all movies high whereas some sceptic users give low ratings to most of the movies. We include this remark to predictions by defining a subject bias.
        \end{itemize}
        ...
        \begin{stattable}{table_flixster}{Statistics for the reduced Flixster data set}
            \hline
            418 & users\\
            \hline
            4237 & movies\\
    ...
    \section{Travel agency}
        ...
        \begin{itemize}
            ...
            \item View profile time. As in the data set there were recorded the events of opening and closing a page, with timestamps, we used the data to determine time the user spent on the tour profile. The longer the time, the higher was the preference according to the rule. 
            ...
            \item Positive object bias according to the amount of implicit feedback. Tours that were much actively visited were supposed to be more likely to be preferred by any user.
        \end{itemize}
        
        After processing the data set in the described way it had the parameters described in the table \ref{table_travel}
        
        \begin{stattable}{table_travel}{Statistics for the Travel agency data set}
            \hline
            193 & users\\
            \hline
            753 & tours\\
...
\chapter{Evaluating Recommender Results}
    ...
    \section{Evaluation Methodology}
        ...
        As we only had access to the presented systems through the data sets, we couldn't perform a live experiment with real system users. Instead of that, we used an offline analysis as described in \cite{evaluating}. We ran Unresyst on a subset of available data (\emph{training data}) and evaluated the results on the rest (\emph{testing data}). In the database on which we performed the recommender build, we kept only the training data. Later on we took the testing data and measured the performance of the evaluator using metrics described in the section \ref{evaluation_metrics}. 
        ...
        \subsection{Selecting Test Data}
            ...
            We used a constant ratio of testing to training data ($1:4$) for all data sets. In data sets where the timestamp in the feedback relationships was available (Last.fm and Travel agency), we selected the given part of the newest relationships as training data. Then the evaluation was done as if we ``replayed'' the scrobbles and orders recorded in the data set. In the Flixster data set where the timestamp wasn't available, we selected the ratings to the test set randomly.
    ...
    \section{Evaluation Results on the Datasets}
        ...
        \subsection{Last.fm}
            ...
            The baseline random recommender gave a rank of $0.5$ as expected. The Slope One recommender wasn't much successful at the data set, its result was almost the same as the result of the random recommender. A breakthrough was done by adding clusters for user gender. Obviously, some music is popular within groups of users of the same gender. The country cluster performed just a bit better then the random recommender. 
            
            Recommending the most listened tracks wasn't much successful, giving results just below the random recommender. On the opposite, recommending artists listened by users with similar age was much more successful. The proposed rule with gender-specific tags didn't bring much success, obviously a better analysis should be performed.
        ...
        \subsection{Flixster}
            ...
            For the Flixster data set, we used the RMSE metric, as there's explicit feedback available. The results can be seen on the chart \ref{pic_chart_flixster}. As a baseline we took a random recommender, that predicted for all pairs expectancy value $0.5$. All tested configurations were better than this baseline. 
            
            \fig[15.5]{pics/chart_flixster.png}{The evaluation results on the Flixster data set. Smaller RMSE values are better.}{pic_chart_flixster}
            ...
            Then we experimented with combining biases using the three combination functions: Average (as a baseline), Certainty Factor and Twisted average, as described in the section \ref{combining}. Twisted average outperformed the remaining ones. The twisted average bias combination was even better than the Slope One recommender.
            
            Then we added the friends relationship, which didn't make much difference. Finally we experimented with mixing the Unresyst knowledge based recommender with the Slope One recommender. The hybrid recommender used Slope One predictions for pair where no rules could be applied. This approach slightly improved the mere Unresyst recommender. Lastly, we tested the knowledge based recommender above Slope One as proposed in the chapter \ref{combining_knowledge_and_cf}. The evaluation results confirms that the predictions of the knowledge based recommender can be successfully used as an input of a collaborative filtering algorithm. 
            
            A table of all evaluation results for the Flixster data set can be found in the Appendix table \ref{table_eval_flixster}. 

            
        \subsection{Travel Agency}
            \label{travel_agency}
            
            For the travel agency data set we used the $\overline{Rank}$ metric as described in the section \ref{evaluation_methodology}. The baseline random recommender result was around $0.5$ which was the expected value.
            
            \fig[15.5]{pics/chart_travel.png}{The evaluation results on the Travel data set. Smaller $\overline{Rank}$ values are better.}{pic_chart_travel}
            ...
            Adding Slope One to a hybrid recommender as well as using Slope One below Unresyst, didn't bring any significant improvement. Obviously the collaborative filtering methods aren't much useful for data sets with sparse data, even when we try to increase the density by a knowledge based recommender.
            
            A table of all evaluation results for the travel agency data set can be found in the Appendix table \ref{table_eval_travel}. 
...
\chapter{Conclusion and Future Work}
    ...
    \section{Future Work}
        ...
        \paragraph{Learning Weights of the Rules} The concept of automatic rule weight setting presented in the section \ref{improving_the_results} can be further studied and expanded, so that the weight setting is done during the build of the recommender. Experiments with more sophisticated learning methods can be performed.
        
        \paragraph{Learning New Rules} Additionally to learning weights of the existing rules, data mining methods can be used for finding new rules in parent system data. The found rules can be discussed with domain experts and eventually added to the Unresyst configuration.
...
%\bibitem{lamport94}
%  \textsc{Lamport}, Leslie. dalsi oddelovat ;
%  \emph{\LaTeX: A Document Preparation System}.
%  2. vydání.
%  Massachusetts: Addison Wesley, 1994.
%  ISBN 0-201-52983-1.
    \bibitem{last_fm}
    \emph{About Last.fm recommendation service}. 
    Last.fm Ltd.[cit. 2011-04-11]
    Available at URL:\url{<http://www.last.fm/about>}
    
    \bibitem{probabilistic_analysis}
    \textsc{Kumar}, Ravi; \textsc{Raghavan}, Prabhakar; \textsc{Rajagopalan} Sridhar; \textsc{Tomkins} Andrew.
    \emph{Recommendation systems: a probabilistic analysis}. 
    39th Annual Symposium on Foundations of Computer Science 1998.
    Available at URL:\url{<http://www.tomkinshome.com/site_media/papers/papers/KRR+98.pdf>}.
    ISBN 0-8186-9172-7.
    
    \bibitem{collaborative_filtering}
    \textsc{Schafer}, Ben; \textsc{Frankowski}, Dan, \textsc{Herlocker}, Joe; \textsc{Sen}, Shilad. 
    \emph{Collaborative Filtering Recommender Systems}. 
    The Adaptive Web, LNCS 4321, 291 - 324, 2007.
    Available at URL:\url{<http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.130.4520&rep=rep1&type=pdf>}.
    
    \bibitem{google_news}
    \textsc{Abhinandan}, Das; \textsc{Mayur}, Datar; \textsc{Ashutosh} Garg; \textsc{Rajaram}, Shyam.
    \emph{Google News Personalization: Scalable Online Collaborative Filtering}. 
    WWW 2007 / Track: Industrial Practice and Experience 2007.
    Available at URL:\url{<http://iws.seu.edu.cn/resource/Proceedings/WWW/2007/papers/paper570.pdf>}.
    ACM 978-1-59593-654-7/07/0005. 
    
    \bibitem{amazon}
    \textsc{Linden}, Greg; \textsc{Smith}, Brent; \textsc{York}, Jeremy.
    \emph{Amazon.com Recommendations Item-to-Item Collaborative Filtering}.
    Published by the IEEE Computer Society 2003.
    Available at URL:\url{<http://www.win.tue.nl/~laroyo/2L340/resources/Amazon-Recommendations.pdf>}.
    IEEE Computer Society 1089-7801/03.
    
    \bibitem{yahoo}
    \textsc{Park}, Seung-Taek; \textsc{Pennock}, David M.
    \emph{Applying collaborative filtering techniques to movie search for better ranking and browsing}. 
    KDD '07 Proceedings of the 13th ACM SIGKDD international conference on Knowledge discovery and data mining, 2007.
    Available at URL:\url{<http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.90.8109&rep=rep1&type=pdf>}.
    ISBN 978-1-59593-609-7.
    
    \bibitem{duine}
    \emph{The Duine Recommender Framework}. 
    Telematica Instituut/Novay. [cit. 2011-04-11]
    Available at URL:\url{<http://duineframework.org/index.html>}
    
    \bibitem{mahout}
    \emph{Overview of Mahout}. 
    Apache Software Foundation.[cit. 2011-04-11]
    Available at URL:\url{<https://cwiki.apache.org/confluence/display/MAHOUT/Overview>}
    
    \bibitem{survey}
    \textsc{Adomavicius}, Gediminas; \textsc{Tuzhilin}, Alexander. 
    \emph{Towards the Next Generation of Recommender Systems: A Survey of the State-of-the-Art and Possible Extensions}.
    IEEE Transactions On Knowledge and Data Engineering, vol. 17, no. 6, June 2005.
    Available at URL: \url{<http://citeseer.ist.psu.edu/viewdoc/download?doi=10.1.1.107.2790&rep=rep1&type=pdf>} 
    IEEE Computer Society 1041-4347/05
    
    \bibitem{netflix_prize}
    \textsc{Bennett}, James, \textsc{Lanning}, Stan. 
    \emph{The Netflix Prize}. 
    KDDCup'07, August 12, 2007.
    Available at URL:\url{<http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.117.8094&rep=rep1&type=pdf>}
    ACM 978-1-59593-834-3/07/0008
    
    \bibitem{netflix_solution} 
    \textsc{Koren}, Yehuda.
    \emph{The BellKor Solution to the Netflix Grand Prize}.
    Netflix prize documentation, 2009.
    Available at URL:\url{<http://www.netflixprize.com/assets/GrandPrize2009_BPC_BellKor.pdf>}
    
    \bibitem{netflix_benefit} 
    \textsc{Bell}, Robert M.; \textsc{Koren} Yehuda.
    \emph{Lessons from the Netflix prize challenge}. 
    ACM SIGKDD Explorations Newsletter 2007, 10.1145/1345448.1345465. 
    Available at URL:\url{<http://www.sigkdd.org/explorations/issues/9-2-2007-12/6-Netflix-1.pdf>}
    
    \bibitem{netflix_end}
    \textsc{Hunt}, Neil.
    \emph{Netflix Prize Update}. 
    Netflix Blog 2010 [cit. 2011-04-11].
    Available at URL: \url{<http://blog.netflix.com/2010/03/this-is-neil-hunt-chief-product-officer.html>}.
    
    \bibitem{google_api}
    \emph{The Google Prediction API}. 
    Google [cit. 2011-04-11]. 
    Available at URL:\url{<http://code.google.com/apis/predict/>}.
    
    \bibitem{aura}
    \emph{The Advanced Universal Recommendation Architecture (AURA)}. 
    Oracle Labs [cit. 2011-04-11].
    Available at URL:\url{<http://labs.oracle.com/projects/dashboard.php?id=196>}.
    
    \bibitem{aura_wiki}
    \emph{The AURA project wiki}. 
    Oracle Corporation [cit. 2011-04-11].
    Available at URL:\url{<http://kenai.com/projects/aura/pages/Home>}.
    
    \bibitem{patent}
    \textsc{Hueter} Geffrey J.; \textsc{Quandt}, Steven C.; \textsc{Hueter}, Noble H.
    \emph{Universal system and method for representing and predicting human behavior}. 
    \#20090248599, United States Patent Application Publication, 2009.
    Available at URL:\url{<http://www.freepatentsonline.com/20090248599.pdf>}.

    \bibitem{peska}
    \textsc{Pe¹ka}, Ladislav.
    \emph{User preferences in the domain of web shops}.
    Master thesis (Mgr.), Charles University in Prague, Faculty of Mathematics and Physics, 2010.
    
    \bibitem{white_paper}
    \textsc{Kunegis}, Jerome; \textsc{Said}, Alan; \textsc{Umbrath}, Winfried.
    \emph{White Paper: The Universal Recommender}. 
    Arxiv preprint arXiv:0909.3472, 2009. 
    Available at URL:\url{<http://uni-koblenz.de/~kunegis/paper/kunegis-universal-recommender.pdf>}.
    
    \bibitem{wiki_genome}
    \textsc{Glaser}, William T.; \textsc{Westergren}, Timothy B.; \textsc{Stearns}, Jeffrey P.; \textsc{Kraft}, Jonathan M.
    \emph{Consumer item matching method and system}. 
    United States Patent 7003515. 
    Available at URL:\url{<http://www.freepatentsonline.com/7003515.pdf>}.
    
    \bibitem{eckhardt} 
    \textsc{Eckhardt}, Alan; \textsc{Horváth}, Tomá¹; \emph{Vojtá¹}, Peter. 
    \emph{PHASES: A user Profile Learning Aproach for Web Search}.
    WI '07, 780-783. IEEE Computer Society Washington. 
    Available to subscribers at URL:\url{<http://portal.acm.org/citation.cfm?id=1331740.1331829>}
    ISBN 0-7695-3026-5
    
    \bibitem{bellkor_ieee}
    \textsc{Koren}, Yehuda; \textsc{Bell}, Robert; \textsc{Volinsky}, Chris.
    \emph{Matrix factorization techniques for recommender systems}.
    Published by the IEEE Computer Society, 0018-9162/09, 2009.
    Available at URL:\url{<http://research.yahoo.com/files/ieeecomputer.pdf>}
    
    \bibitem{knowledge_spain}
    \textsc{Martínez}, Luis; \textsc{Pérez}, Luis G.; \textsc{Barranco}, Manuel J.; \textsc{Espinilla}, M.
    \emph{A Knowledge Based Recommender System Based on Consistent Preference Relations}.
    Studies in Computational Intelligence, 2008, Volume 117/2008, 93-111, DOI: 10.1007/978-3-540-78308-4\_6 
    Available for subscribers at URL: \url{<http://www.springerlink.com/content/m64g474m7p0t4r26/>}.
    
    \bibitem{burke_hybrid}
    \textsc{Burke}, Robin.
    \emph{Hybrid Web Recommender Systems}.
    The Adaptive Web, LNCS 4321, pp. 377 - 408, 2007.
    Available at URL: \url{<http://josquin.cs.depaul.edu/twiki/pub/Csc594/ArticleList/burke-hybrid.pdf>}
    
    \bibitem{knowledge_burke}
    \textsc{Burke}, Robin.
    \emph{Knowledge-based recommender systems}.
    Department of Information and Computer Science, University of California, Irvine 200, To Appear in the Encyclopedia of Library and Information Science.
    Available at URL: \url{<http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.41.3078&rep=rep1&type=pdf>}
    
    \bibitem{integrating_burke}
    \textsc{Burke}, Robin.
    \emph{Integrating Knowledge-based and Collaborative-filtering Recommender Systems}.
    AAAI Technical Report WS-99-01, 1999, AAAI.
    Available at URL: \url{<http://www.aaai.org/Papers/Workshops/1999/WS-99-01/WS99-01-011.pdf>}.
    
    \bibitem{order}
    \textsc{Cohen}, William W.; \textsc{Schapire}, Robert E.; \textsc{Singer}, Yoram.
    \emph{Learning to order things}.
    Journal of Artificial Intelligence Research 10 (1999) 234-270.
    Available at URL: \url{<http://www.jair.org/media/587/live-587-1790-jair.pdf>}
    
    \bibitem{racofi}
    \textsc{Anderson}, Michelle; \textsc{Lemire}, Daniel; \textsc{Ball}, Marcel; \textsc{Boley}, Harold; \textsc{Greene}, Stephen; \textsc{Howse}, Nancy; \textsc{McGrath}, Sean.
    \emph{RACOFI: A Rule-Applying Collaborative Filtering System}.
    Proc. IEEE/WIC COLA'03, Halifax, Canada, October 2003. 
    Available at URL: \url{<http://www.daniel-lemire.com/fr/documents/publications/racofi_nrc.pdf>}
    NRC 46507
    
    \bibitem{last_fm_dataset1}
    \textsc{Celma}, Oscar.
    \emph{Last.fm Dataset - 1K users}.
    [cit. 2011-04-11].
    Available at URL: \url{<http://www.dtic.upf.edu/~ocelma/MusicRecommendationDataset/lastfm-1K.html>}.
    
    \bibitem{last_fm_dataset2}
    \textsc{Lamere}, Paul.
    \emph{LastFM-ArtistTags2007}, Dataset.
    [cit. 2011-04-11].
    Available at URL: \url{<http://musicmachinery.com/2010/11/10/lastfm-artisttags2007/>}.
    
    \bibitem{flixster_dataset}
    \textsc{Jamali}, Mohsen.
    \emph{Flixster Data Set}.
    [cit. 2011-04-11]
    Available at URL: \url{<http://www.cs.sfu.ca/~sja25/personal/datasets/>}.
    
    \bibitem{flixster_paper}
    \textsc{Jamali}, Mohsen; \textsc{Ester}, Martin.
    RecSys '10 Proceedings of the fourth ACM conference on Recommender systems 
    \emph{A matrix factorization technique with trust propagation for recommendation in social networks}
    Available for subscribers at URL: \url{<http://portal.acm.org/citation.cfm?id=1864708.1864736>}
    ISBN: 978-1-60558-906-0

    \bibitem{ruleml_short}
    \textsc{Boley}, Harold.
    \emph{POSL: An Integrated Positional-Slotted Language for Semantic Web Knowledge}
    Faculty of Computer Science University of New Brunswick, Working Draft 11 May 2004.
    Available at URL: \url{<http://ruleml.org/submission/ruleml-shortation.html>}
    
    \bibitem{mycin}
    \textsc{Shortliffe}, Edward Hance.
    \emph{Computer-based medical consultation MYCIN}. 
    America Elsevier Publishing Company, Inc., New York 1976.
    Available as a book at URL: \url{<http://www.annals.org/content/85/6/831.1.extract>}
    ISBN 978-0444001795
    
    \bibitem{certainty_calculus}
    \textsc{Lucas}, P.J.F.
    \emph{Certainty-factor-like structures in Bayesian belief networks}. 
    Knowledge-Based Systems 14, 2001, 327-335
    Available at URL:\url{<http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.19.1853&rep=rep1&type=pdf>}
    PII S 0950-705 1(00)00073-3
    
    \bibitem{design_patterns}
    \textsc{Gamma}, Erich; \textsc{Helm}, Richard; \textsc{Johnson} Ralph; \textsc{Vlissides}, John M.
    \emph{Design Patterns: Elements of Reusable Object-Oriented Software}.
    Addison-Wesley Professional; 1 edition (November 10, 1994)
    ISBN 978-0201633610
    
    \bibitem{evaluating}
    \textsc{Herlocker}, Jonathan L.; \textsc{Konstan}, Joseph A.; \textsc{Terveen}, Loren G.; \textsc{Riedl}, John T.
    \emph{Evaluating collaborative filtering recommender systems}. 
    ACM Transactions on Information Systems (TOIS), Volume 22 Issue 1, January 2004.
    Available at URL:\url{<http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.97.5270&rep=rep1&type=pdf>}
    
    \bibitem{rmse_critic}
    \textsc{McNee}, Sean M.; \textsc{Riedl}, John; \textsc{Konstan}, Joseph A.
    \emph{Being accurate is not enough: how accuracy metrics have hurt recommender systems}. 
    CHI 2006, April 22-27, 2006, Montreal, Canada. 
    Available at URL: \url{<http://www.grouplens.org/papers/pdf/mcnee-chi06-acc.pdf>}
    
    \bibitem{ndcg}
    \textsc{Valizadegan}, Hamed; \textsc{Jin}, Rong; \textsc{Zhang}, Ruofei; \textsc{Mao}, Jianchang.
    \emph{Learning to Rank by Optimizing NDCG Measure}.
    Advances in Neural Information Processing Systems 22, 1883-1891, 2009.
    Available at URL:\url{<http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.154.8402&rep=rep1&type=pdf>}.
    
    \bibitem{koren_implicit}
    \textsc{Hu}, Yifan; \textsc{Koren}, Yehuda; \textsc{Volinsky}, Chris.
    \emph{Collaborative Filtering for Implicit Feedback Datasets}.
    2008 Eighth IEEE International Conference on Data Mining.
    Available at URL: \url{<http://www2.research.att.com/~yifanhu/PUB/cf.pdf>}
    ISBN: 978-0-7695-3502-9
    
    \bibitem{slope_one}
    \textsc{Lemire}, Daniel; \textsc{Maclachlan}, Anna.
    \emph{Slope One Predictors for Online Rating-Based Collaborative Filtering}
    SIAM Data Mining (SDM'05), Newport Beach, California, April 21-23, 2005
    Available at URL: \url{<http://arxiv.org/pdf/cs.DB/0702144>}
    arXiv:cs/0702144v2
...
%%% Pou¾ité zkratky v diplomové práci, existují-li, vèetnì jejich vysvìtlení.
\chapwithtoc{List of Abbreviations and Terms}
    \begin{description}
        \item[API] Application programming interface, a set of methods implemented by a software system that can 
        be used by another system.
        \item[CSV] Comma-separated values, a simple format of text files where the values are separated by commas.
        \item[GUI] Graphical User Interface, a type of user interface that uses graphics when communicating with user, rather than textual commands.
        \item[ORM] Object-relational mapping, a technique used for manipulating data directly in the application without using SQL.
        \item[RMSE] Root Mean Square Error, a metric for evaluating accuracy of recommender systems.
        \item[Scrobble] Scrobble is a verb used in the Last.fm system. To scrobble means to send information about what song a user is playing to the Last.fm server.
        \item[SQL] Structured Query Language a language for data manipulation in a database.
        \item[Unresyst] Universal Recommender System proposed in the thesis.
        \item[XML] Extensible Markup Language, a format of text documents in machine-readable form.
    \end{description}
...
\chapwithtoc{Attachments}
    ...
    \begin{enumerate}
        \item Unresyst CD
        \item List of Tools and Other Resources Used
        \item Recommender Evaluation Result Tables
    \end{enumerate}

    \section*{Unresyst CD}
        The CD is fixed to the thesis book cover. The full list of its contents and brief instructions on how to use it, can be found in the file \verb!readme.txt! on the CD in the root folder. The CD contains:
        \begin{enumerate}
            \item Complete source codes of the Unresyst implementation prototype.
            \item The source code of the Slope One recommender, using the Mahout framework, including the framework setup files and scripts for running the recommender on Unresyst exported data.
            \item Scripts for reducing the data sets and saving data from them into the system database.
            \item Generated programming documentation to the Unresyst implementation prototype.
            \item Complete data models both for Unresyst and applications covering the data sets.
            \item This text in the \emph{PDF} format.
            \item Other resources as diagrams and presentations.
        \end{enumerate}
        
        All the named files can be found on the Unresyst project homepage at URL:  \url{<http://code.google.com/p/unresyst/>}.
\newpage
        \section*{List of Tools and Other Resources Used}
        
        During the work on the thesis we used the following software tools and resources:
        
        \begin{enumerate}
            \item \emph{Python 2.6.5} for building the prototype implementation.
            \item The ORM layer of the \emph{Django 1.2} web application framework, to operate with the database.
            \item The \emph{MySQL 14.14} database (can be interchanged with any other database supported by the Django framework).
            \item The \emph{gedit} text editor for writing the source code.
            \item The \emph{pdfTeXk 1.40.3} for building the PDF from \LaTeX text.
            \item \emph{Kile 2.0.85} for writing the thesis text in \LaTeX.
            \item The drawings were overtaken from the \emph{Tim Tim picture database} of freely available drawings. The drawings are available at URL: \url{<http://www.timtim.com/>}.
            \item For hosting the source codes in the \emph{SVN} repository, we used the \emph{Google Code} service. Instructions on how to checkout the repository are available at URL: \url{<http://code.google.com/p/unresyst/source/checkout>}.
        \end{enumerate}

        All the tools are freely available, under open source licenses. The named services are free. 
        
        The source code of the prototype implementation is available under \emph{Mozilla Public License 1.1}, see URL: \url{<http://www.mozilla.org/MPL/>}. Other contents as the thesis text are available under \emph{Creative Commons 3.0 BY-SA}, see URL: \url{<http://creativecommons.org/licenses/by-sa/3.0/>}.
\newpage
        \section*{Recommender Evaluation Result Tables}

        The attachment contains complete lists of experiments done on the datasets. The interpretation of the results including charts can be found in the section \ref{evaluating_recommender_results}.
        \begin{table}[ht]
            \begin{center}
            \footnotesize
                \label{table_eval_lastfm}
                \begin{tabular}{|l|r|}
                    \hline
                    \textbf{Recommender Setup} & \textbf{$\overline{Rank}$} \\
                    \hline
                    \hline
                    ...
                    \hline
                        Unresyst, combination of successful rules & \\
                        gender cluster (weight 0.5), country cluster(weight 0.1), & \\
                        similar age (0.4) & 0.184017 \\
                    \hline
                        Unresyst, combination of successful rules & \\
                        above Slope One & \\
                        gender cluster (weight 0.5), country cluster(weight 0.1), & \\
                ...
                \end{tabular}
                \caption{Results for evaluating recommenders on the Last.fm data set.}
            \end{center}
        \end{table}
        ...
        \begin{table}[ht]
            \begin{center}
            \footnotesize
                \label{table_eval_flixster}
                \begin{tabular}{|l|r|}
                
                    \hline
                        \textbf{Recommender Setup} & \textbf{$RMSE$} \\
                    \hline
                    \hline
                    ...
                    \hline
                        Unresyst, all biases (weights 1.0 pos., 0.25 neg.), friends (weight 0.65),& \\
                        average & 0.245660 \\
                    \hline
                        Unresyst, all biases (weights 1.0 pos., 0.25 neg.), friends (weight 0.65), & \\
                        certainty factor & 0.239204 \\
                    \hline
                        Unresyst, all biases (weights 1.0 pos., 0.25 neg.), friends (weight 0.65), & \\
                        twisted average & 0.227061 \\
                    \hline
                        Unresyst, all biases (weights 1.0 pos., 0.25 neg.), friends (weight 0.1), & \\
                        twisted average & 0.218237 \\
                    \hline
                        Hybrid of Unresyst and Slope One, all biases& \\
                        (weights 1.0 pos., 0.25 neg.),  friends (weight 0.1), twisted average & 0.207439 \\
                    \hline
                        Slope One below Unresyst, all biases (weights 1.0 pos., 0.25 neg.), & \\
                        friends (weight 0.1), twisted average & 0.206578 \\
                    \hline
                \end{tabular}
                \caption{Results for evaluating recommenders on the Flixster data set.}
            \end{center}
        \end{table}
            ...
            \begin{center}
                ...
                \begin{tabular}{|l|r|}
                    ...
                    \hline
                        Unresyst with bias - most mouse moved tours & 0.326599\\
                    \hline
                        Unresyst with combination mouse move (weight 0.1), view profile (weight 0.1), & \\
                        click (weight 0.05), bias viewed (weight 0.3), bias clicked (weight 0.25), & \\
                        bias mouse moved (weight 0.2), average & 0.250765\\
                    \hline
                        Unresyst with combination mouse move (weight 0.1), view profile (weight 0.1), & \\
                        click (weight 0.05), bias viewed (weight 0.3), bias clicked (weight 0.25), & \\
                        bias mouse moved (weight 0.2), certainty factor & 0.250765\\
                    \hline
                        Unresyst with combination of mouse move (weight 0.1), view profile (weight 0.1), & \\
                        click (weight 0.05), bias viewed (weight 0.3), bias clicked (weight 0.25), & \\
                        bias mouse moved (weight 0.2), twisted average  & 0.250765\\
                    \hline
                        Hybrid of Unresyst and Slope One with combination of mouse move (weight 0.1), & \\
                        view profile (weight 0.1), click (weight 0.05), bias viewed (weight 0.3), & \\
                        bias clicked (weight 0.25), bias mouse moved (weight 0.2), & \\
                        twisted average  & 0.250765\\
                    \hline
                        Slope One below Unresyst with combination of mouse move (weight 0.1), & \\
                        view profile (weight 0.1), click (weight 0.05), bias viewed (weight 0.3), & \\
                        bias clicked (weight 0.25), bias mouse moved (weight 0.2), & \\
                        twisted average  & 0.251148 \\
                    \hline
                \end{tabular}
                \caption{Results for evaluating recommenders on the travel agency data set.}
            \end{center}
        \end{table}
