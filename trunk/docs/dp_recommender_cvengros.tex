...
\chapter{Recommender Analysis}
    \label{recommender_analysis}
    In the chapter we discuss the circumstances under which recommender systems are a preferrable option both for users and system holders. Later in the chapter we describe the reasons for using our universal recommender (\emph{Unresyst}), we compare our recommender to existing solutions. Finally we formally define the problem of recommending.
    
    Recommender system is a part of a web-based application, that uses data about users and their behaviour to provide users with  items which are the most relevant to them. The recommended items are typically things that are liable to user taste, like books, music, news, etc.
    
    \section{Recommender System Benefits}
        \label{recommender_system_usage}
        ...
        \subsection{Benefits for a System Holder}
            ...
            Another interesting feature recommender systems provide to e-shop holders is the ability to include promoted items to recommendations. E-shop holders can artificially increase the recommendation rates of the promoted goods in order they apeear in recommendations for more users. However, this feature should be used moderately because it distorts the results of the recommender algorithm. A massive propagation of items that are promoted by the system holder but are not very interesting for particular users may lead to a loss of user's trust for the recommender.
            
            And last but not least, a recommender system provieds a competitive advantage. Web business leaders like Google or Amazon have already implemented recommenders in their systems, for smaller companies a recommender system may be a key advantage over their rivals.
    
    \section{Universal Recommender Analysis}
        \label{universal_recommender_analysis}
        In the section we describe the position of the proposed recommender system (\emph{Unresyst}) on the market. We depict advantages and disadvantages of the Unresyst recommdender, we compare Unresyst to existing sollutions. Finally we propose processes for incorporating Unresyst to a web-based system and for running a system involving Unresyst.
        ...
        \subsection{Universal Recommender Applicability}
            ...
            Data that can be used by Unresyst include but are not limited to:
            \begin{description}
                \item[Demographic information] both user and items
                \item[User behaviour]
                \item[Explicit feedback]
                \item[Social relations]
                \item[Other user attributes]
                \item[Other item attributes]
             \end{description}
        \subsection{Comparison to Existing Solutions}
        
        \subsection{Unresyst Process Proposal}
            depends on particular company, environment, brief
    ...
    \section{Comparison}
        ...
        Some systems provide advanced item search 
        - user activity
        - user knows what he wants
        - taste 
        - sorted by relevance to search criteria not by preference
